...
const Discount = () => {
  ...
  useEffect(()=>{
    ...
    const fetchCategories=async()=>{
        try {
            const response=await axiosClient.get(`api/discount?page=${currentPage}`);
            if(response.status==="failed"){
                console.log("failed fetch",response.data.message)
            }else{
                setCat(response.data.message.data)
    ...
    const  confirmDelete=async(id)=>{
        const response=await axiosClient2.delete(`api/discount/${id}`);
  ...
  return (
    ...
    <div className='col-lg-10 col-md-10 col-sm-10'>
        <Link to="/admin/create_discount" className='btn  ms-4 mt-4 text-white' style={{fontWeight:"bold",background:"rgb(129, 216, 94)"}}> <FontAwesomeIcon className='text-white' icon={faPlus} />Add Discount</Link>
            ...
            <tbody>
            {
                cat?.map((e)=>(
                    <tr key={e.id}>
                        ...
                        <td>{e.code}</td>
                        <td >{e.percentage} %</td>
                        <td >{(e.created_at).replace('.000000Z'," ").replace('T',"  :: ")} </td>
                        <td>
                            ...
                            <Link   to={`/admin/update_discount/${e.id}`} className='btn m-1' style={{fontWeight:"bold",display:"inline-block" ,borderRadius:'5px',background:"green" ,color:"white" }}> 
